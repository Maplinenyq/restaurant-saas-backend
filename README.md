...
| 模块 | 说明 |
|------|------|
| **nyq-common** | 公共模块 — 常量、上下文、枚举、异常、工具类、统一响应格式 |
| **nyq-pojo** | 数据对象模块 — Entity（表映射）、DTO（数据传输）、VO（视图对象） |
| **nyq-server** | 服务模块 — Controller、Service、Mapper、配置、拦截器、启动类 |
...
```sql
CREATE DATABASE nyq_take_out CHARACTER SET utf8mb4;
```

导入项目 SQL 文件：

```bash
mysql -u root -p nyq_take_out < nyq-server/src/main/resources/db/sky.sql
```

### 2. 修改配置

编辑 `nyq-server/src/main/resources/application.yml`，配置数据库、Redis、JWT：

```yaml
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/nyq_take_out
    username: root
...
```bash
mvn clean package
java -jar nyq-server/target/nyq-server-*.jar
```

或直接在 IDE 中运行 `NyqApplication.java`。
